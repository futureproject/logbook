class Api::V2::EngagementAttendeesController < Api::V2::BaseController
  ...
  def create
    ...
    if @engagement_attendee.save
      render :show, status: :created, location: api_v2_engagement_url(@engagement_attendee)
    else
  ...
  def update
    if @engagement_attendee.update(engagement_params)
      render :show, status: :ok, location: api_v2_engagement_url(@engagement_attendee)
    else
      render json: @engagement_attendee.errors, status: :unprocessable_entity
    end
  end
